...
export async function generateMetadata({
  ...
  if (n === null) notFound();
  return getBrowseIndexMetadata("museums", n);
}

export default async function MuseumsPaginatedPage({
  params,
}: {
  params: Promise<{ page: string }>;
}) {
  ...
  const n = parsePage(page);
  if (n === null) notFound();
  if (n === 1) permanentRedirect("/browse/museums");

  return <BrowseIndexContent kind="museums" page={n} />;
}
